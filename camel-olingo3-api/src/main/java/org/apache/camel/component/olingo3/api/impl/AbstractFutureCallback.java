 ...
 * for {@link org.apache.camel.component.olingo3.api.impl.Olingo3AppImpl}
*/
public abstract class AbstractFutureCallback<T> implements FutureCallback<HttpResponse> {
    ...
    private final Olingo3ResponseHandler<T> responseHandler;

    AbstractFutureCallback(Olingo3ResponseHandler<T> responseHandler) {
            ...
            if (response.getEntity() != null) {
                try {
                    ...
                    if (ODATA_MIME_TYPE_PATTERN.matcher(responseContentType.toContentTypeString()).matches()) {
                        final ODataReader reader = ODataClientFactory.getV3().getReader();
                        final ODataError error = reader.readError(response.getEntity().getContent(), ODataFormat.fromContentType(responseContentType));
